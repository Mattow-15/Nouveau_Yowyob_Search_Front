...
// ── Composant principal ────────────────────────────────────────────────────
// Le produit arrive déjà résolu depuis le Server Component parent (fetch SSR
// pour le premier rendu + generateMetadata) — plus de fetch client au montage.
...
export function ProductDetailClient({ product }: ProductDetailClientProps) {
    ...
    const productActualLocation = useMemo<[number, number]>(() => [
        product.latitude || product.location?.lat || 3.848,
        product.longitude || product.location?.lng || 11.5021,
    ], [product]);
    ...
    const mapMarkers = useMemo(() => [{
        id: product.id,
        position: productActualLocation,
